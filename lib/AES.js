...
class AES {
  constructor() {
     ...
     */
    this.set_key = function (key_data) {
   ...
   /**
    ...
    this.encrypt_block = function (start_chunk) {
      ...
      AES_AddRoundKey(block_data, key.slice(0, 16));
      for (var i = 16; i < l - 16; i += 16) {
        AES_SubBytes(block_data, AES_Sbox);
        AES_ShiftRows(block_data, AES_ShiftRowTab);
        AES_MixColumns(block_data);
        AES_AddRoundKey(block_data, key.slice(i, i + 16));
      }
      AES_SubBytes(block_data, AES_Sbox);
      AES_ShiftRows(block_data, AES_ShiftRowTab);
      AES_AddRoundKey(block_data, key.slice(i, l));
    ...
    this.decrypt_block = function (start_chunk) {
      let block = start_chunk;
      if (this.iv_set == true) {
      ...
      AES_AddRoundKey(block_data, key.slice(l - 16, l));
      AES_ShiftRows(block_data, AES_ShiftRowTab_Inv);
      AES_SubBytes(block_data, AES_Sbox_Inv);
      for (var i = l - 32; i >= 16; i -= 16) {
        AES_AddRoundKey(block_data, key.slice(i, i + 16));
        AES_MixColumns_Inv(block_data);
        AES_ShiftRows(block_data, AES_ShiftRowTab_Inv);
        AES_SubBytes(block_data, AES_Sbox_Inv);
      }
      AES_AddRoundKey(block_data, key.slice(0, 16));
