...
const writeUInt32LE = (array, value, index) => {
...
class BLOWFISH {
  constructor() {
    ...
    var i, index;
    for (let i = 0; i < 4; i++) {
        for (index = 0; index < S[i].length; index++) {
            writeUInt32LE(this.buffer, S[i][index], (i * 1024) + (index * 4))
        }
    }
    ...
    for (index = 0; index < P.length; index++) {
        writeUInt32LE(this.buffer, P[index], (index * 4) + offsetP)
    }
    ...
    this.enc_round = function () {
        var tl, tr, temp;
        const offsetP = 4096;
        tl = this.xl;
        tr = this.xr;
        for (i = 0; i != 16; ++i) {
            tl = (tl ^ readUInt32LE(this.buffer, (i * 4) + offsetP)) >>> 0;
            tr = (this.F(tl) ^ tr) >>> 0;
            temp = tl;
            tl = tr;
            tr = temp;
    ...
    this.dec_round = function () {
        var tl, tr, temp;
        const endoffsetP = 4164;
        tl = this.xl;
        tr = this.xr;
        for (i = 0; i != -16; --i) {
            tl = (tl ^ readUInt32LE(this.buffer, (i * 4) + endoffsetP)) >>> 0;
            tr = (this.F(tl) ^ tr) >>> 0;
            temp = tl;
            tl = tr;
            tr = temp;
