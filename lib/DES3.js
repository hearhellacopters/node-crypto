...
class DES3{
    constructor() {
        ...
        this.BLOCK_SIZE = 8;
        this.KEY_LENGTH = 8;
        this.sKey = new Uint32Array(32);
        this.INTERNAL_KEY_LENGTH = this.ROUNDS * 2;
        this.SKB = new Uint32Array(8 * 64);
        this.SP_TRANS = new Uint32Array(8 * 64);

        this.__static_initializer_0 = function () {
            ...
            for (var i = 0; i < cd.length; i++) {
                {
                    s = (function (c) { return c.charCodeAt == null ? c : c.charCodeAt(0); })(cd.charAt(i)) - '@'.charCodeAt(0);
                    if (s !== 32) {
                        bit = 1 << count++;
                        for (j = 0; j < 64; j++) {
                            if ((bit & j) !== 0)
            ...
            for (var i = 0; i < 32; i++) {
                {
                    ...
                    for (j = 0; j < 32; j++) {
                        {
                            c = (function (c) { return c.charCodeAt == null ? c : c.charCodeAt(0); })(spt.charAt(offset >> 1)) - '0'.charCodeAt(0) >> (offset & 1) * 3 & 7;
                            offset++;
                            if (c < 5) {
                                k += c + 1;
                                this.SP_TRANS[k] |= bit;
                            ...
                            param = (function (c) { return c.charCodeAt == null ? c : c.charCodeAt(0); })(spt.charAt(offset >> 1)) - '0'.charCodeAt(0) >> (offset & 1) * 3 & 7;
                            offset++;
                            if (c === 5) {
                                k += param + 6;
                                this.SP_TRANS[k] |= bit;
        ...
        this.__static_initialized = true
         ...
         *
         * @param  {byte[]} block       an array containing the input block
         * @param  {boolean} encrypt  true to encrypt, false to decrypt
         * @param {number} outOffset
         */
        this.run_block = function (block, encrypt) {
         ...
         * @private
         */
        /*private*/ this.encrypt_base = function (io) {
            var L = io[0];
            var R = io[1];
            var u = R << 1 | R >>> 31;
            R = L << 1 | L >>> 31;
         ...
         * @private
         */
        /*private*/ this.decrypt_base = function (io) {
            var L = io[0];
            var R = io[1];
            var u = R << 1 | R >>> 31;
            R = L << 1 | L >>> 31;
        ...
        /*private*/ this.initialPermutation = function (io) {
            var L = io[0];
            var R = io[1];
        ...
        /*private*/ this.finalPermutation = function (io) {
            var L = io[1];
            var R = io[0];
