...
class CAST128 {
    ...
    f1(_d, _r, _m) {
        ...
        return (((((this.S2[BYTE2(t)] ^ this.S1[HIBYTE(t)]) >>> 0) - this.S3[BYTE1(t)]) >>> 0) + this.S4[BYTE(t)]) >>> 0;
    }
    f2(_d, _r, _m) {
        var t = this.rotL((_d ^ _r) >>> 0, _m);
        return ((this.S1[HIBYTE(t)] - this.S2[BYTE2(t)] + this.S3[BYTE1(t)]) ^ this.S4[BYTE(t)]) >>> 0;
    }
    f3(_d, _r, _m) {
    ...
    set_key(key) {
        ...
        let x0 = bswap32(readUInt32LE(key, 0));
        let x1 = bswap32(readUInt32LE(key, 4));
        let x2 = bswap32(readUInt32LE(key, 8));
        let x3 = bswap32(readUInt32LE(key, 12));
        const X = [x0, x1, x2, x3];
        var x = key;
        var z = [];
        var Z = [];
        ...
        K.forEach((el, i) => {
            writeUInt32LE(this.buffer, el, i * 4);
        });
        this.key_set = true;
    }
    ;
    ...
    encrypt_block(block) {
        ...
        if (this.iv_set == true) {
            start_chunk = xor_switch(block, this.iv);
        }
        let tx = readUInt32LE(start_chunk, 0);
        let ty = readUInt32LE(start_chunk, 4);
        const D = [tx, ty];
        var i;
        for (i = 0; i < 16; i++) {
            let a2 = D[(i + 1) % 2];
            let a3 = readUInt32LE(this.buffer, i * 4);
        ...
        writeUInt32LE(x, D[0] >>> 0, 0);
        writeUInt32LE(x, D[1] >>> 0, 4);
        if (this.iv_set == true) {
            this.iv = x;
        }
        var bulkout = reverse64(x);
        return bulkout;
    }
    ;
    decrypt_block(block) {
        let start_chunk = bswap64(block);
        if (this.iv_set == true) {
        ...
        writeUInt32LE(x, D[1] >>> 0, 0);
        writeUInt32LE(x, D[0] >>> 0, 4);
        if (this.iv_set == true) {
            x = xor(x, this.iv);
            var new_iv;
            if (isBuffer(block)) {
            ...
            writeUInt32LE(new_iv, ty, 0);
            writeUInt32LE(new_iv, tx, 4);
            this.previous_block = new_iv;
        }
        var return_buffer = bswap64(x);
