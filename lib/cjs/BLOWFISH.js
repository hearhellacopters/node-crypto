...
class BLOWFISH {
    ...
    set_key(key) {
        ...
        for (i = 0; i < 4; i++) {
            for (index = 0; index < this.S[i].length; index++) {
                (0, common_1.writeUInt32LE)(this.buffer, this.S[i][index], (i * 1024) + (index * 4));
            }
        }
        for (index = 0; index < this.P.length; index++) {
            (0, common_1.writeUInt32LE)(this.buffer, this.P[index], (index * 4) + this.offsetP);
        }
        var blocks = key.length / 4;
        var v11 = [];
        for (index = 0; index < blocks; index++) {
            var element = (0, common_1.readUInt32LE)(key, index * 4);
            v11.push(element);
        }
        for (index = 0; index < 18; index++) {
            var loc = (index * 4) + this.offsetP;
            var element = (0, common_1.readUInt32LE)(this.buffer, loc);
            (0, common_1.writeUInt32LE)(this.buffer, (element ^ v11[index & (blocks - 1)]) >>> 0, loc);
        }
        this.xl = 0;
        this.xr = 0;
        for (index = 0; index < 18; index = index + 2) {
            ...
            this.enc_round();
            (0, common_1.writeUInt32LE)(this.buffer, this.xl >>> 0, loc);
            (0, common_1.writeUInt32LE)(this.buffer, this.xr >>> 0, loc + 4);
        }
        const offsetS = 0;
    ...
    F(x) {
        ...
        y = (0, common_1.readUInt32LE)(this.buffer, (0 * 1024) + (a * 4)) + (0, common_1.readUInt32LE)(this.buffer, (1 * 1024) + (b * 4));
        y = y ^ (0, common_1.readUInt32LE)(this.buffer, (2 * 1024) + (c * 4));
        y = y + (0, common_1.readUInt32LE)(this.buffer, (3 * 1024) + (d * 4));
        return y;
    }
    ;
    enc_round() {
        ...
        tr = this.xr;
        for (var i = 0; i != 16; ++i) {
            tl = (tl ^ (0, common_1.readUInt32LE)(this.buffer, (i * 4) + this.offsetP)) >>> 0;
            tr = (this.F(tl) ^ tr) >>> 0;
            temp = tl;
            tl = tr;
            tr = temp;
        ...
        tl = tr;
        tr = temp;
        tr = (tr ^ (0, common_1.readUInt32LE)(this.buffer, 4160)) >>> 0;
        tl = (tl ^ (0, common_1.readUInt32LE)(this.buffer, 4164)) >>> 0;
        this.xl = tl;
        this.xr = tr;
    }
    ;
    dec_round() {
        ...
        tr = this.xr;
        for (var i = 0; i != -16; --i) {
            tl = (tl ^ (0, common_1.readUInt32LE)(this.buffer, (i * 4) + this.endoffsetP)) >>> 0;
            tr = (this.F(tl) ^ tr) >>> 0;
            temp = tl;
            tl = tr;
            tr = temp;
        ...
        tl = tr;
        tr = temp;
        tr = (tr ^ (0, common_1.readUInt32LE)(this.buffer, 4100)) >>> 0;
        tl = (tl ^ (0, common_1.readUInt32LE)(this.buffer, 4096)) >>> 0;
        this.xl = tl;
        this.xr = tr;
    }
    ;
    ...
    decrypt_block(block) {
        ...
        this.xl = (0, common_1.readUInt32LE)(start_chunk, 0);
        this.xr = (0, common_1.readUInt32LE)(start_chunk, 4);
        this.dec_round();
        var x;
        ...
        (0, common_1.writeUInt32LE)(x, this.xl, 0);
        (0, common_1.writeUInt32LE)(x, this.xr, 4);
        if (this.iv_set == true) {
            x = (0, common_1.xor)(x, this.iv);
    ...
    encrypt_block(block) {
        ...
        if (this.iv_set == true) {
            start_chunk = (0, common_1.xor_switch)(block, this.iv);
        }
        this.xl = (0, common_1.readUInt32LE)(start_chunk, 0);
        this.xr = (0, common_1.readUInt32LE)(start_chunk, 4);
        this.enc_round();
