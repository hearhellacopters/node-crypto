...
export class BLOWFISH {
    ...
    private buffer = new Uint8Array(4168);

    private offsetP = 4096;

    private endoffsetP = 4164;
    ...
    set_key(key:Buffer|Uint8Array):void {
        ...
        for (i = 0; i < 4; i++) {
            for (index = 0; index < this.S[i].length; index++) {
                writeUInt32LE(this.buffer, this.S[i][index], (i * 1024) + (index * 4))
            }
        }
    
        for (index = 0; index < this.P.length; index++) {
            writeUInt32LE(this.buffer, this.P[index], (index * 4) + this.offsetP)
        }
        ...
        for (index = 0; index < blocks; index++) {
            var element = readUInt32LE(key, index * 4);
            v11.push(element);
        }

        for (index = 0; index < 18; index++) {
            var loc = (index * 4) + this.offsetP;
            var element = readUInt32LE(this.buffer, loc);
            writeUInt32LE(this.buffer,(element ^ v11[index & (blocks - 1)]) >>> 0,  loc)
        }
        ...
        for (index = 0; index < 18; index = index + 2) {
            ...
            this.enc_round();
            writeUInt32LE(this.buffer, this.xl >>> 0, loc);
            writeUInt32LE(this.buffer, this.xr >>> 0, loc + 4);
        }
    ...
    F(x:number):number {
        ...
        y = readUInt32LE(this.buffer, (0 * 1024) + (a * 4)) + readUInt32LE(this.buffer, (1 * 1024) + (b * 4));
        y = y ^ readUInt32LE(this.buffer, (2 * 1024) + (c * 4));
        y = y + readUInt32LE(this.buffer, (3 * 1024) + (d * 4));

        return y;
    };

    enc_round():void {
        var tl:number, tr:number, temp:number;
        tl = this.xl;
        tr = this.xr;
        for (var i = 0; i != 16; ++i) {
            tl = (tl ^ readUInt32LE(this.buffer, (i * 4) + this.offsetP)) >>> 0;
            tr = (this.F(tl) ^ tr) >>> 0;
            temp = tl;
            tl = tr;
            tr = temp;
        ...
        tl = tr;
        tr = temp;
        tr = (tr ^ readUInt32LE(this.buffer, 4160)) >>> 0;
        tl = (tl ^ readUInt32LE(this.buffer, 4164)) >>> 0;
        this.xl = tl;
        this.xr = tr;
    };
    ...
    dec_round():void {
        ...
        tr = this.xr;
        for (var i = 0; i != -16; --i) {
            tl = (tl ^ readUInt32LE(this.buffer, (i * 4) + this.endoffsetP)) >>> 0;
            tr = (this.F(tl) ^ tr) >>> 0;
            temp = tl;
            tl = tr;
            tr = temp;
        ...
        tl = tr;
        tr = temp;
        tr = (tr ^ readUInt32LE(this.buffer, 4100)) >>> 0;
        tl = (tl ^ readUInt32LE(this.buffer, 4096)) >>> 0;
        this.xl = tl;
        this.xr = tr;
    };
    ...
    decrypt_block(block:Buffer|Uint8Array):Buffer|Uint8Array {
        ...
        this.xl = readUInt32LE(start_chunk, 0);
        this.xr = readUInt32LE(start_chunk, 4);
        ...
        writeUInt32LE(x, this.xl, 0)
        writeUInt32LE(x, this.xr, 4);
        if (this.iv_set == true) {
            x = xor(x, this.iv);
    ...
    encrypt_block (block:Buffer|Uint8Array):Buffer|Uint8Array {
